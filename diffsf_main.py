...
from utils.particle_utils import compute_epe2
...
class Trainer(object):
    def __init__(
        ...
        # lr_scheduler
        # 这里的step指的是scheduler.step()的次数，600000 / 1135 = 528
        # 初期lr会下降地比较慢，后续加快，不适合调参？
        ...
        self.logger = Logger(self.lr_scheduler, summary_writer, summary_freq=500, start_step=self.epoch)
    ...
    def train(self):
        ...
        while self.epoch < self.train_num_epochs:
            ...
            # if get_rank() == 0: 
            #     wandb.log({'Loss': np.array(loss_train).mean()})
            ...
            # if (self.epoch < 300 and (self.epoch+1) % 50 == 0) or (self.epoch >= 300 and (self.epoch+1) % 5 == 0):
...
class Tester(object):
    def __init__(
        ...
        # self.polynomialRegressor = PolynomialRegressor(degree=2, alpha=0.1)
    ...
    def polyfit(self, data, degree=2, alpha=10.0):
        ...
        for b in range(data.shape[0]):
            for i in range(data.shape[3]):
                ...
                X = poly_features.fit_transform(t.reshape(-1, 1))  # 将时间索引转换为多项式特征
                y = data[b, -num_frames:, :, i]  # 目标值
                ...
                # 预测下一个帧
                next_t = np.array([num_frames]).reshape(1, -1)  # 下一个时间点的多项式特征
    ...
    def test(self):
        ...
        with torch.inference_mode():
            ...
            for i, data in enumerate(val_progress):
                ...
                particle_mask = data['particle_mask']
                particle_mask = particle_mask.to(torch.long).to(device)
                ...
                #     np.savez("preds/"+filename, **{'pred': flow_pred[index].cpu().numpy()})

                for index in range(len(filenames)):
                    filename = filenames[index].split('/')[-1]
                    # print(index, flow_pred.shape, flow_3d.shape, points_list.shape, pcs.shape)
                    np.savez("visualization_norm/"+filename, **{'flows_pred': flow_pred[index].cpu().numpy(), 'flows_gt': flow_3d[index].cpu().numpy(), 
                                                                 'points_pred': points_list[index].cpu().numpy(), 'points_gt': pcs[index].cpu().numpy(),
                                                                 'particle_mask': particle_mask[index].cpu().numpy()})

                # for frame_index in range(flow_3d.shape[1]):
                #     epe, acc3d_strict, acc3d_relax, outlier = compute_epe2(torch.permute(flow_3d[:, frame_index], (0, 2, 1)).cpu().numpy(), 
                #                                                         torch.permute(flow_pred[:, frame_index], (0, 2, 1)).cpu().numpy(), 
                #                                                         flow_3d_mask[:, frame_index].cpu().numpy())
                #     multi_metircs_run[frame_index]['EPE'].append(epe)
                #     multi_metircs_run[frame_index]['ACC_Relax'].append(acc3d_relax)
                #     multi_metircs_run[frame_index]['ACC_Strict'].append(acc3d_strict)
                #     multi_metircs_run[frame_index]['outlier'].append(outlier)

                flow_3d_zeros = torch.zeros_like(flow_3d_mask, dtype=torch.int64).to(device)
                for bs in range(particle_mask.shape[0]):
                    for nf in range(particle_mask.shape[1]):
                        flow_3d_mask[bs, nf, particle_mask[bs, nf, :]] = flow_3d_zeros[bs, nf, particle_mask[bs, nf, :]]
                        if nf != 0:
                            flow_3d_mask[bs, nf, particle_mask[bs, nf-1, :]] = flow_3d_zeros[bs, nf, particle_mask[bs, nf-1, :]]
                epe, acc3d_strict, acc3d_relax, outlier = compute_epe2(torch.permute(flow_3d, (0, 1, 3, 2)).cpu().numpy(), 
                                                                       torch.permute(flow_pred, (0, 1, 3, 2)).cpu().numpy(), 
                                                                       flow_3d_mask.cpu().numpy())
                epe_run.append(epe)
            ...
            results['Acc3dStrict'] = np.array(acc3dStrict_run).mean() * 100.0
            print("Validation Things EPE: %.6f, Acc3dStrict: %.6f, Acc3dRelax: %.4f, Outlier: %.4f" % (results['EPE'], results['Acc3dStrict'], results['Acc3dRelax'], results['Outlier']))
            # for frame_index, metric in enumerate(multi_metircs_run):
            #     print("Frame %d: EPE: %.6f, Acc3dStrict: %.6f, Acc3dRelax: %.4f, Outlier: %.4f" % (frame_index+1, np.array(metric['EPE']).mean(), 
            #                                                                                                     np.array(metric['ACC_Strict']).mean(), 
            #                                                                                                     np.array(metric['ACC_Relax']).mean(), 
            #                                                                                                     np.array(metric['outlier']).mean()))
...
if __name__ == '__main__':
    ...
    if args.eval:
        tester = Tester(
            ...
            resume = args.resume,                # total training epochs
    ...
    # train
        ...
        trainer = Trainer(
            ...
            test_batch_size = args.test_batch_size,
            train_lr = args.lr,
            train_num_epochs = args.num_epochs,         # total training epochs
            results_folder = args.result_dir,
