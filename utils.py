...
def compute_epe(est_flow, batch):
    ...
    acc3d_strict = (
        (np.logical_or(l2_norm < 0.05, relative_err < 0.05)).astype(np.float64).mean()
    )
    acc3d_relax = (
        (np.logical_or(l2_norm < 0.1, relative_err < 0.1)).astype(np.float64).mean()
    )
    outlier = (np.logical_or(l2_norm > 0.3, relative_err > 0.1)).astype(np.float64).mean()

    return EPE3D, acc3d_strict, acc3d_relax, outlier

def compute_epe2(sf_pred, sf_gt, mask):
    sf_pred = sf_pred[mask > 0]
    sf_gt = sf_gt[mask > 0]
    ...
    acc3d_strict = (
        (relative_err < 0.05).astype(np.float64).mean()
    )

    acc3d_relax = (
        (relative_err < 0.1).astype(np.float64).mean()
    )
    outlier = (np.logical_or(l2_norm > 0.3, relative_err > 0.1)).astype(np.float64).mean()
